...
| Desk sweep | hourly | every desk: `../../AGENT_WORKSPACE/AGENTS/DESKS/*/` and `../DESK/` | moves files marked `review_status: sweep` to `../../AGENT_WORKSPACE/TRASH/Desk_Sweep/[SEAT]/`, where `[SEAT]` is the desk's own folder name verbatim — `GUIDES_DESK`, `DIRECTORS_DESK`, `OVERSEERS_DESK`, and `OPERATOR_DESK` for the operator's `DESK/` |
...
jobs — routine chores that run on a clock instead of inside a chat. Two
are planned from day one: a daily refresh of the master index and an
hourly sweep that carries flagged desk files to the trash. The Overseer
builds them during its first setup session, but the schedule is yours: nothing
