...
Spawn `AGENTS/SUB_AGENTS/OVERSEER/RELAY.md` — one per slice, in parallel. Single-shot, scoped, no memory: the completion report IS its memory, and it carries no state to the next file or the next slice.
...
> A RELAY instance cannot repoint a `Protected` file in place — its own rules force it to surface. A census weighted toward Protected fixtures comes back to the Overseer for patch-then-promote, one at a time; do not plan a fan-out around them.
...
`TRASH/` and its sweeps · `STASIS/` · `PROJECTS/PARKED/` · `PROJECTS/GRAVEYARD/` · `../OPERATOR_WORKSPACE/ARCHIVE/` · static memory cards · session logs
...
| Rule |
|---|
| At cascade start, open `[CHANGE]_CASCADE_LEDGER.md` on the Overseer desk — census, slices, cleared vs pending |
| Update it as slices land — the ledger, not session memory, is where a half-done cascade survives a close |
| All cascades cleared → trash it. A ledger with open rows never trashes |
| A row a cron self-heals is never escalated and never holds the ledger open |
...
> A fan-out that returns zero in-class targets still costs a full run. Before spending it, check the census was built on the token that CHANGES — a census built on a token that survives the change finds nothing by construction.
