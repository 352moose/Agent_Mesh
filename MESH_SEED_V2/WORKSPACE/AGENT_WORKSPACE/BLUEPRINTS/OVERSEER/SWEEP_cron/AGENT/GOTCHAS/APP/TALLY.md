...
| x   | Gotcha | Fix |
| --- | ------ | --- |
| 1   | a foreign tree sat behind an OS privacy boundary the agent shell could not cross, and moving execution to the agent looked like the fix | the SCHEDULER holds the access grant; execution stays with the scheduler and the asymmetry is the design, not the bug |
