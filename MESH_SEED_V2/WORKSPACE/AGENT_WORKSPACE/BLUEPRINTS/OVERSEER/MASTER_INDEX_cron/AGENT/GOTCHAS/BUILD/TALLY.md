...
| x   | Gotcha | Fix |
| --- | ------ | --- |
| 1   | a hand-run writes the same log line as the scheduler and has masked a totally dead cron | verify with read-only calls to the pure functions; liveness = a REAL scheduled tick advancing the log, nothing else counts |
