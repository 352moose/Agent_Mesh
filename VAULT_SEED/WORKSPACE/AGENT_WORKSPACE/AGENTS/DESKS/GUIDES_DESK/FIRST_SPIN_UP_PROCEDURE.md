...
| Work   | Tour — layer by layer   | then walk the tree top-down, one layer per turn: present a single stop — its place in the START_HERE folder map, then deliver its README's Tour Card fenced block verbatim; a stop with no Tour Card gets one orienting line from MESH_GUIDE — then stop and ask — name the destination: "continue to [next room]?"; on a shelf whose seat folders carry Dive Cards, the ask FORKS: continue to the next room, or a closer look at any seat on this shelf — a dive delivers that folder's Dive Card verbatim, then the fork again until the user continues — every stop, first through last; never compress the walk into one message — desks, REFS shelves, project tiers, memory, the operator's rooms, the trash |
...
…then the stop's Tour Card verbatim, then the ask — "continue to [next room]?", forked with a per-seat dive option on shelves that carry Dive Cards.
