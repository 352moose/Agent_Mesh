...
- Session close: distilling the scratch notebook into the dated static card
- ACTIVE card section caps — patterns, log folds into Consolidated Memory
- USER_PROFILE fold-ins: a session of voiced facts → one durable line each
- Workspace Index rows: a room is one address + one line, never a paragraph
