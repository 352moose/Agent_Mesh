...
| 4 | List every hit by filename, classify live / fossil / coincidence. Fossils (ARCHIVE, BACKUPS, TRASH, PARKED, GRAVEYARD, static cards, session logs) keep the old path — expected, not drift |
| 5 | Move — `git mv` inside a repo, copy-then-verify otherwise. The move itself is one step of seven, not the job |
| 6 | Repoint live references in batches, diffing each batch |
