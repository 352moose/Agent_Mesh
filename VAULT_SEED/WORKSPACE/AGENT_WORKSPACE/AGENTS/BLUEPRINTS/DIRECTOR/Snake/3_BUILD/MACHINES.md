...
> **No machine has built this.** Every fork in `PROCEDURE.md` is authored and unproven, including the default `python3` + Tk path. Add your row when cycle 1 completes — not when all three do, because a machine that got a window open and then stopped is still evidence about that machine.
